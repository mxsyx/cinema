""" 从站点 “www.yunbtv.com” 中提取综艺节目主页的地址，
...
class VH(scrapy.Spider):
    ...
    def parse(self, response):
        try:
            ...
            with open('tmp/variety/%d.json' % self.current, "w") as f:
                json.dump(homepages_dump, f)
        except Exception as e:
            settings.logerr.error("@VH.parse URL: %s  Exception: %s" % (self.url,str(e)))
        ...
        if(self.current <= self.increment_max):
            self.url = "https://www.yunbtv.com/vodtype/zongyi-%d.html" % self.current
            yield scrapy.Request(self.url, callback=self.parse)
        else:
            self.task_completed()

    def task_completed(self):
        merlist.comerge("variety", self.increment, self.increment_max)  # 合并刚刚存储的URL
        print("\n\n\n任务完成！ 综艺节目 - 主页地址提取完毕\n\n\n")
        self.ema.send("任务完成消息","综艺节目 - 主页地址提取完毕")