...
class TU(scrapy.Spider):
    ...
    def __init__(self, increment_low=0, increment_high=0, *args, **kwargs):
        ...
        Args:
            ...
            increment_high  终止条目
        """
        super(TU, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        # 此处从tvseries.all.json获取URL以更新数据
        with open("tmp/tvseries.all.json","r") as url_file:
    ...
    def parse(self, response):
        ...
        parse函数重新解析了每一条电视剧的播放地址，并将更新信息传输到存储器pipelines
        """
        item = TvseriesItem()
    ...
    def extract_urls(self,response):
        ...
        try:
            ...
            settings.logerr.error("@TU.extract_url URL: %s  Exception: %s" % (response.url, str(e)))
    ...
    def chrome_extract(self,_url):
        ...
        try:
            ...
            settings.logerr.error("@TU.chrome_extract URL: %s  Exception: %s" % (_url, str(e)))
            return "none"
    ...
    def task_completed(self):
        self.driver.quit()
        settings.loginfo.info("任务完成！ 电视剧 - 信息更新完毕\n")
        print("\n\n\n任务完成！ 电视剧 - 信息更新完毕\n\n\n")
        self.ema.send("任务完成消息","电视剧 - 信息更新完毕")