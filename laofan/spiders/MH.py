...
class MH(scrapy.Spider):
    ...
    def __init__(self, start_url=None, increment=0, increment_max=0, *args, **kwargs):
        ...
        Args:
            ...
            increment_max  终止条目
        """
        super(MH, self).__init__(*args, **kwargs)
        self.url = start_url
    ...
    def parse(self, response):
        try:
            ...
            with open('tmp/movie/%d.json' % self.current, "w") as f:
                json.dump(homepages_dump, f)
        except Exception as e:
            settings.logerr.error("@MH.parse URL: %s  Exception: %s" % (self.url,str(e)))
        ...
        if(self.current <= self.increment_max):
            self.url = "https://www.yunbtv.com/vodtype/dianying-%d.html" % self.current
            yield scrapy.Request(self.url, callback=self.parse)
        else:
            self.task_completed()

    def task_completed(self):
        merlist.comerge("movie", self.increment, self.increment_max)  # 合并刚刚存储的URL
        print("\n\n\n任务完成！ 电影 - 主页地址提取完毕\n\n\n")
        self.ema.send("任务完成消息","电影 - 主页地址提取完毕")