...
class AP(scrapy.Spider):
    ...
    def start_requests(self):
        # 此处从anime.new.json获取URL以逐条解析数据
        with open("tmp/anime.new.json","r") as url_file:
    ...
    def parse(self, response):
        ...
        parse函数解析了每一条动漫主页地址，将与动漫有关的信息例如名字、简介、年代、
        播放地址等信息提取出来封装到 AnimeItem 类中，并将结果传送到了存储器pipelines
    ...
    def extract_name(self,response):
        try:
            ...
            settings.logerr.error("@AP.extract_name URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_introduction(self,response):
        try:
            ...
            settings.logerr.error("@AP.extract_introduction URL: %s  Exception: %s" % (response.url, str(e)))
            return "无"
    ...
    def extract_director(self,response):
        try:
            ...
            settings.logerr.error("@AP.extract_director URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_flag_time(self,response):
        try:
            ...
            settings.logerr.error("@AP.extract_flag_time URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_flag_type(self,response):
        try:
            ...
            settings.logerr.error("@AP.extract_flag_type URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_img(self,response):
        ...
        try:
            ...
            img_dir = "imga/%s/" % time.strftime("%Y-%m-%d", time.localtime())
            if os.path.isdir(img_dir)==False:
                os.mkdir(img_dir)
            urlretrieve(result, "%s%s" % (img_dir, img_name))    # 下载图片到指定位置
            return "/media/%s%s" % (img_dir, img_name)
        except Exception as e:
            settings.logerr.error("@AP.extract_img URL: %s  Exception: %s" % (response.url, str(e)))
    ...
    def extract_urls(self,response):
        ...
        try:
            ...
            settings.logerr.error("@AP.extract_url URL: %s  Exception: %s" % (response.url, str(e)))
    ...
    def chrome_extract(self,_url):
        ...
        try:
            ...
            settings.logerr.error("@AP.chrome_extract URL: %s  Exception: %s" % (_url, str(e)))
            return "none"
    ...
    def task_completed(self):
        self.driver.quit()
        print("\n\n\n任务完成！ 动漫 - 信息提取完毕\n\n\n")
        self.ema.send("任务完成消息","动漫 - 信息提取完毕")