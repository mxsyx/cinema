""" 从站点 “www.yunbtv.com” 中提取电视剧主页的地址，
...
class TH(scrapy.Spider):
    ...
    def parse(self, response):
        try:
            ...
            with open('tmp/tvseries/%d.json' % self.current, "w") as f:
                json.dump(homepages_dump, f)
        except Exception as e:
            settings.logerr.error("@TH.parse URL: %s  Exception: %s" % (self.url,str(e)))
        ...
        if(self.current <= self.increment_max):
            self.url = "https://www.yunbtv.com/vodtype/dianshiju-%d.html" % self.current
            yield scrapy.Request(self.url, callback=self.parse)
        else:
            self.task_completed()

    def task_completed(self):
        merlist.comerge("tvseries", self.increment, self.increment_max)
        print("\n\n\n任务完成！ 电视剧 - 主页地址提取完毕\n\n\n")
        self.ema.send("任务完成消息","电视剧 - 主页地址提取完毕")