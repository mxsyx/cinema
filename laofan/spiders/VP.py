...
class VP(scrapy.Spider):
    ...
    def __init__(self, increment_low=0, increment_high=0, *args, **kwargs):
        ...
        Args:
            ...
            increment_high  终止条目
        """
        super(VP, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        # 此处从variety.new.json获取URL以逐条解析数据
        with open("tmp/variety.new.json","r") as url_file:
    ...
    def parse(self, response):
        ...
        parse函数解析了每一条综艺节目主页地址，将与综艺节目有关的信息例如名字、简介、年代、
        播放地址等信息提取出来封装到 VarietyItem 类中，并将结果传送到了存储器pipelines
    ...
    def extract_name(self,response):
        try:
            ...
            settings.logerr.error("@VP.extract_name URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_introduction(self,response):
        try:
            ...
            settings.logerr.error("@VP.extract_introduction URL: %s  Exception: %s" % (response.url, str(e)))
            return "无"
    ...
    def extract_director(self,response):
        try:
            ...
            settings.logerr.error("@VP.extract_director URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_flag_time(self,response):
        try:
            ...
            settings.logerr.error("@VP.extract_flag_time URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_flag_type(self,response):
        try:
            ...
            settings.logerr.error("@VP.extract_flag_type URL: %s  Exception: %s" % (response.url, str(e)))
            return "未知"
    ...
    def extract_img(self,response):
        ...
        try:
            ...
            img_dir = "imgv/%s/" % time.strftime("%Y-%m-%d", time.localtime())
            if os.path.isdir(img_dir)==False:
                os.mkdir(img_dir)
            urlretrieve(result, "%s%s" % (img_dir, img_name))    # 下载图片到指定位置
            return "/media/%s%s" % (img_dir, img_name)
        except Exception as e:
            settings.logerr.error("@VP.extract_img URL: %s  Exception: %s" % (response.url, str(e)))
    ...
    def extract_urls(self,response):
        ...
        try:
            ...
            settings.logerr.error("@VP.extract_url URL: %s  Exception: %s" % (response.url, str(e)))
    ...
    def chrome_extract(self,_url):
        ...
        try:
            ...
            settings.logerr.error("@VP.chrome_extract URL: %s  Exception: %s" % (_url, str(e)))
            return "none"
    ...
    def task_completed(self):
        self.driver.quit()
        print("\n\n\n任务完成！ 综艺节目 - 信息提取完毕\n\n\n")
        self.ema.send("任务完成消息","综艺节目 - 信息提取完毕")